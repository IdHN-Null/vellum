/** v0.3.0 rendering-pipeline visual verification harness */
...
const base = generateBase(map); // noise + erosion
const baseMs = performance.now() - t0;

const t1 = performance.now();
const terrain = composeTerrain(map, base, null, null); // hydrology + classification
...
// Verify the chunked (tile) render matches the full render
{
...
// Verify continent/island count control
...
addLabel("v0.3 프리미엄 합성 — 침식 지형·유량 테이퍼 강·호수·배치 요소 4종");
{
  ...
  // Hand-drawn bathymetry/contours
  let sc = 999;
  ...
  // Rivers: the same 2-pass approach as the plugin's drawVectorLines (water wash + thin ink line)
  {
  ...
  // Test region (tan — a case that used to sink into the background)
  ...
  // Test annotations: dashed path + arrows (treasure-map markings)
  ...
  // Record the arrow tip pixel for inspection
  ...
  for (const m of markers) {
    ...
    // Name tag: the same 'written directly on the map' style as view.paintMarker
...
// Colour-style paper texture + new arrowhead check
...
// Sharpness (LOD) check: a 4× zoomed region — left: blurry upscale / right: detailed re-render
addLabel("선명도(LOD): 좌=흐린 업스케일 · 우=상세 재렌더(높이 샘플링)");
{
  const pal = getPalette("parchment");
  const zoom = 4, size = 300;
  const cx0 = terrain.w * 0.4, cy0 = terrain.h * 0.4;     // crop's top-left cell
  const cells = size / zoom;                               // number of cells shown on screen
  const row = document.createElement("div");
  row.style.cssText = "display:flex;gap:10px";
  document.body.appendChild(row);

  // (Left) upscale the cell-resolution base (imageSmoothing=true → blurry)
  ...
  // (Right) per-pixel detailed re-render (bilinear height + nearest biome + refined coast)
  ...
  for (let j = 0; j < size; j++) {
    for (let i = 0; i < size; i++) {
      ...
      if (!terrain.lake[ci]) { // lakes are exempt from height reclassification (stops them vanishing when zoomed)
...
// Async worker generation check: worker result == sync result
(() => {
  ...
  w.onmessage = (e) => {
    ...
    // Pixel comparison with the sync result (sampled)
    let diff = 0;
...
// Vanishing-lake bug check: with the bug (no guard) water turns to land; with the fix (guard) it stays water
(() => {
  // Generate lake-prone terrain (noise mode)
  ...
  for (let i = 0; i < lt.lake.length; i++) {
    ...
    // (Bug) reclassification without the guard
    let bb = b;
    if (el >= sea && (bb === B.DEEP || bb === B.OCEAN)) bb = B.BEACH;
    if (bb === B.DEEP || bb === B.OCEAN) buggyWater++;
    // (Fix) lakes are exempt from reclassification → always water
    fixedWater++;
  }
  const st = window as unknown as Record<string, unknown>;
  st.__lakeCells = lakeCells;
  st.__lakeBuggyWater = buggyWater;   // cells left as water under the bug (most lakes expected to turn to land)
